...
library StormLib {
    ...
    enum MsgType { ANCHOR, UNCONDITIONAL, SHARDED, SETTLE, SETTLESUBSET, ADDFUNDSTOCHANNEL} //, SINGLECHAIN, MULTICHAIN } TODO: uncomment if using singleswap
    ...
    function anchor(bytes calldata message, bytes calldata signatures, address owner, mapping(uint => Channel) storage channels, mapping(address => FeeStruct) storage tokenAmounts) external returns (uint) {
        ...
        require(MsgType(uint8(message[0])) == MsgType.ANCHOR, "p");
    ...
    //takes in the balance, fee, and the amount owed the counterparty, and the new balance, and the new fee OWED TO KALADIN
    //same methodology done for owner, partner
    //owner fees calculated
        //if less than or equal to total balance, pay them out to Kaladin, partner, return
        //if greater than total balance:
            //if Kaladin fee less or equal to the total balance:
                //pay Kaladin fee. Pay remainder to partner. Set owner bal to 0
            //else (kaladin fee > balance):
                //set Kaladin fee equal to the total balance, partner fee to 0, owner fee to 0
    //NOTE: fees aren't added to each other accts until the end. There is no situation, say, where partner not able to pay funds, but can with the addition of owner funds first. No particular reason done this way, could be changed.
    function feeLogic(uint userBalance, uint fees) private pure returns (uint, uint, uint) {
        uint feesDue = uint(fees / FEE_DENOM_TOTAL);
        uint counterPartyAccrued = 0;
        if (feesDue <= userBalance) {
            fees = uint(feesDue / FEE_DENOM_KAL); //this is now set to track their fees paid to Kaladin, and their reward in Kaladimes.
            counterPartyAccrued = feesDue - fees;
            userBalance -= feesDue;
        } else {
            //dont have enough in channel to pay all fees; pay as much as can, user ends up with 0 for their balance
            uint KaladinFee = uint(feesDue / FEE_DENOM_KAL);
            if (KaladinFee <= userBalance) {
                //there is enough to pay Kaladin fees
                fees = KaladinFee;
                counterPartyAccrued = userBalance - KaladinFee; //partner receives the rest of the money in fees
            } else {
                //Kaladin takes everything that is in channel
                fees = userBalance;
            }
            userBalance = 0;
        }
        return (counterPartyAccrued, userBalance, feesDue);
    }
    ...
    function calcBalsAndFees(bytes calldata message, BalanceStruct memory balanceStruct, uint balanceTotal, address partnerAddr, uint i) private returns (address tokenAddress) {
        ...
        //NOTE: in this, the partner is paid first before owner pays fees.
        uint feeToCounterparty;
        (feeToCounterparty, balanceStruct.ownerBalance, balanceStruct.ownerFee) = feeLogic(balanceStruct.ownerBalance, balanceStruct.ownerFee);
        balanceStruct.partnerBalance += feeToCounterparty;
        (feeToCounterparty, balanceStruct.partnerBalance, balanceStruct.partnerFee) = feeLogic(balanceStruct.partnerBalance, balanceStruct.partnerFee);
        balanceStruct.ownerBalance += feeToCounterparty;
        
        //now, ownerFee and partnerFee correspond to the amount they should pay Kaladin. ownerBalance, partnerBalance correctly updated to reflect fees being paid and fees paid
    ...
    function distributeSettleTokens(bytes calldata message, uint numTokens, mapping(address => FeeStruct) storage tokenAmounts, bool isSettleSubset) private returns (uint160) {
        ...
        return uint160(bytes20(keccak256(abi.encodePacked(balanceTotalsNew))));
    }
    ...
    function settle(bytes calldata message, bytes calldata signatures, address owner, mapping(uint => Channel) storage channels, mapping(address => FeeStruct) storage tokenAmounts) external returns(uint channelID) {
        ...
        distributeSettleTokens(message, numTokens, tokenAmounts, false);
    ...
    function settleSubset(bytes calldata message, bytes calldata signatures, address owner, mapping(uint => Channel) storage channels, mapping(address => FeeStruct) storage tokenAmounts) external returns(uint channelID, uint32 nonce) {
        ...
        channels[channelID].balanceTotalsHash = distributeSettleTokens(message, numTokens, tokenAmounts, true);
    ...
    /** 
     */
    function checkBalanceTotalOverflow(Channel storage channel, bytes calldata message, MsgType msgType, uint numTokens) private {
        ...
        if (msgType == MsgType.SHARDED) {
            ...
            channel.msgHash = uint(keccak256(message[0 : message.length - (numTokens * 32) - ((msgType == MsgType.ANCHOR) ? 32 : 4)])); //We need everything(channelID, token splits, msg data, except for the balanceTotals and the nonce)
        }
    }
    ...
    */
    ...
    function disputeStartedChecks(Channel storage channel, bytes calldata message, bytes calldata signatures, uint numTokens, address owner) private returns (uint32 nonce, MsgType msgType) {
        ...
        require(msgType == MsgType.ANCHOR || msgType == MsgType.UNCONDITIONAL || msgType == MsgType.SHARDED, "p");
        address partnerAddr;
     ...
     */
     //Receives a {ANCHOR, UNCONDITIONAL, SHARDED} || balanceTotalsMsg. Stores in msgHash = {msgType with deadline(ANCHOR), nonce(else) stripped out} || shardDataMsg
    ...
    function startDispute(bytes calldata message, bytes calldata signatures, address owner, mapping(uint => Channel) storage channels) external returns(uint channelID, uint32 nonce, uint numTokens) {
        ...
        checkBalanceTotalOverflow(channel, message, msgType, numTokens);
    ...
    function withdraw(bytes calldata message, mapping(uint => Channel) storage channels, mapping(address => FeeStruct) storage tokenAmounts) external returns (uint channelID) {        
        ...
        for (uint i = 0; i < numTokens; i++) {
            BalanceStruct memory shardTokenBal;
            assembly { calldatacopy(shardTokenBal, add(message.offset, add(startBals, mul(i, BALANCESTRUCT_UNIT))), BALANCESTRUCT_UNIT) }
            shardTokenBals[i] = shardTokenBal;
        }
